...
def run_anova_all_numeric(df):
    ...
    for col in numeric_cols:
        ...
        try:
            ...
            # 儲存為 CSV
            anova_table.to_csv(f"{output_dir}/anova_{col}.csv")
...
def ks_kl_analysis_all_features(df):
    ...
    # 儲存成 CSV
    pd.DataFrame(ks_all).to_csv(f"{output_dir}/ks_test_summary.csv", index=False)
    pd.DataFrame(kl_all).to_csv(f"{output_dir}/kl_divergence_summary.csv", index=False)
...
if __name__ == "__main__":
    ...
    stability_df.to_csv(f"{output_dir}/stability_summary.csv", index=False)
    print("✅ 每週 CV 統計已儲存")
